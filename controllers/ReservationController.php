...
class ReservationController extends YFrontController
{
    ...
    public function actionShow($alias)
    {
        ...
        if (!$company_id) {
            throw new CHttpException(404, Yii::t('booking', 'Запрошенная страница не найдена!'));
        } else {
...
//            'join' => 'INNER JOIN {{booking}} booking ON booking.id=t.booking_id INNER JOIN {{company_company}} company ON company.id=booking.company_id',
        ...
        if ($model) {
            ...
            if ($model->is_protected == Booking::PROTECTED_YES && !Yii::app()->user->isAuthenticated()) {
                $this->redirect(array(Yii::app()->getModule('user')->accountActivationSuccess2));
            }
    ...
    public function actionOrder()
    {
        if(!Yii::app()->user->isAuthenticated()) {
            echo Yii::t('booking', 'Пожалуйста, войдите на сайт под своим именем, чтобы оформить заказ!');
            Yii::app()->end();
        }
        ...
        if (Yii::app()->request->isPostRequest && isset($_POST['BookingOrders'])) {
            ...
            if($whole_item == 1) {
                if($order_date > 0) {
                    echo Yii::t('booking', 'Этот объект уже забронирован!');
                    Yii::app()->end();
                }
            } else {
            ...
            if ($model->save()) {
                if (Yii::app()->request->isAjaxRequest) {
                    echo Yii::t('booking', 'Ваш заказ на бронирование добавлен!');
                    Yii::app()->end();
                } else {
                    Yii::app()->user->setFlash(
                            YFlashMessages::NOTICE_MESSAGE, Yii::t('booking', 'Ваш заказ на бронирование добавлен!')
                    );
                ...
                echo Yii::t('booking', 'Ваш заказ на бронирование не добавлен. Попробуйте позже');
                Yii::app()->end();
            }
        }
